...
class Environment():
    ...
    def __init__(self, ref, num_squares, device):
        ...
        resized = cv.resize(cropped,(GRID_SIZE,GRID_SIZE))
    ...
    def take_action(self, action):
        ...
        #reward = (self.img.size*255-np.sum(abs(self.ref-self.img)))/(self.img.size*255)
        #Mean Square Error reward function
        reward = 1-(np.sum((self.ref-self.img)**2)/self.img.size)
    ...
    def get_state(self):
        ...
        #FOR COLOUR IMAGES THE NP ARRAY WILL HAVE TO BE TRANSPOSED USING (self.ref - self.img).transpose((2,0,1))
        #MAKE SURE RGB not BGR!!
        return torch.from_numpy(self.ref - self.img).type(torch.FloatTensor).unsqueeze(0).to(self.device)
...
class QValues():
    ...
    def get_next(target_net, next_states, action_count, action_limit):
        if action_count >= (action_limit - 1):
            #TODO: set to reward and not to 0
            #batch_size = next_states.shape[0]
            #determine value of the rewards of the final states using mean square error
            #WILL NEED TO BE UPDATED FOR RGB
            final_rewards = 1 - torch.sum(next_states**2,(1,2))/(torch.prod(torch.tensor(next_states[0].size()),0))
            #print (final_rewards)
...
env = Environment("TestImages/TestImage.png", NUM_SQUARES, device)
