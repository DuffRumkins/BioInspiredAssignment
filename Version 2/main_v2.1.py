...
import Constants_v21

IMG_SIZE, SCALE, NUM_SQUARES, NUM_IMAGES = Constants_v21.IMG_SIZE,\
                                           Constants_v21.SCALE,\
                                           Constants_v21.NUM_SQUARES,\
                                           Constants_v21.NUM_IMAGES
...
class DQN(nn.Module):
    def __init__(self, img_height, img_width):
        ...
        self.fc4 = nn.Linear(in_features = 24, out_features= 32)
        self.fc5 = nn.Linear(in_features = 32, out_features = 32)
        
        self.fc6 = nn.Linear(in_features = 32, out_features = 64)
        self.fc7 = nn.Linear(in_features = 64, out_features = 64)

        #output layer is also fully connected and has as many outputs as possible actions (action_space)
        self.out = nn.Linear(in_features = 64, out_features= action_space)"""
        ...
        self.conv2 = nn.Conv2d(in_channels=9, out_channels=12, kernel_size=(3,3), stride=(1,1), padding=(1,1))
        self.conv3 = nn.Conv2d(in_channels=12, out_channels=12, kernel_size=(3,3), stride=(1,1), padding=(1,1))
        self.pool = nn.MaxPool2d(kernel_size = (2,2), stride = (2,2))
        self.fc = nn.Linear(in_features=int(12*(IMG_SIZE/2)**2), out_features=int(12*(IMG_SIZE/2)**2))
        self.out = nn.Linear(in_features = int(12*(IMG_SIZE/2)**2), out_features= action_space)
...
class Environment():
    ...
    def reset(self, new_ref = False):
        """reset the environment. If new_ref is set to True a new reference image will be randomly generated"""
        self.action_count = 0
        self.img = np.zeros(np.shape(self.ref), np.uint8)
        #self.actions_taken = [None]
        self.previous_states = np.zeros((1,self.height,self.width))
    ...
    #define a function to generate a new random image
    def generate_ref(self):
        self.ref = np.zeros((IMG_SIZE,IMG_SIZE), np.uint8)
        ...
        for square in range(NUM_SQUARES):
            ...
            cv.rectangle(self.ref, tuple(square_top_left), tuple(square_bottom_right),
    ...
    def get_state(self):
        ...
        #added extra batch and channel dimensions so states are in (B,C,H,W) this is needed for the CNN
        return torch.from_numpy(self.ref.astype(np.float32) - self.img.astype(np.float32)).type(torch.FloatTensor).unsqueeze(0).unsqueeze(0).to(self.device)
...
class QValues():
    ...
    def get_next(target_net, next_states, action_count, action_limit):
        if action_count >= (action_limit - 1):
            ...
            #UPDATED FOR CONVOLUTIONAL LAYERS!!!!!!!!!
            final_rewards = -torch.sum(next_states**2,(2,3))/(torch.prod(torch.tensor(next_states[0].size()),0))
            #print (final_rewards)
            return final_rewards.squeeze(1)
...
def plot(values, moving_avg_period):
    ...
    #special case for iPython inline display
...
#Hyperparameters 
#NEED TUNING these were taken from cart-pole example
...
policy_net = DQN(IMG_SIZE,IMG_SIZE).to(device)
...
#initialise the environment, strategy and memory --> Can be any image will be reset with random image before running
env = Environment(f"TestImages/TestImage.jpg", NUM_SQUARES, device) #any image can be used to initialise the environment for training
#env = Environment(f"TestImages/TestImage.jpg", NUM_SQUARES, torch.device('cpu'))
memory = ReplayMemory(memory_size)
strategy = EpsilonGreedyStrategy(eps_start, eps_end, eps_decay)
agent = Agent(strategy, action_space, device)
#agent = Agent(strategy, action_space, torch.device('cpu'))



"""
#load checkpoint IF DOING SO REMEMBER TO MANUALLY SET EPSILON
checkpoint = torch.load("DQN/policy_net.pt")
...
while not Trained:
    #create target network based off of policy network
    target_net = DQN(IMG_SIZE,IMG_SIZE).to(device)
    ...
    #TRAINING LOOP
    #iterate through num_episodes episodes. After this the model will be saved and reloaded to perserve RAM memory
    for episode in range(num_episodes):
        #reset the environment and generate new random image
        env.reset(new_ref = True)
        #get starting state
        state = env.get_state()
        ...
        for timestep in count():
            ...
            if memory.can_provide_sample(batch_size):
                ...
                #inputs to the convolutional layers should have the following dimensions (batch_size, #input channels, height, width)
                #! NOT NEEDED ANYMORE SINCE WE ADD EXTRA DIMENSION IN THE GET_STATE FUNCTION
                ...
                #!SQUEEZE TO REMOVE EXTRA DIMENSION FOR CNN VERSION
                #target_q_values = ((next_q_values.squeeze(1) * gamma) + rewards).type(torch.cuda.FloatTensor)
    ...
    #close the environment after the policy network has been trained for num_episodes
    #env.close()

    #save model checkpoint
    torch.save({'model_state_dict': policy_net.state_dict(),
                'optimizer_state_dict' : optimizer.state_dict(),
                'loss': loss}, "DQN/policy_net.pt")
    #load checkpoint
    checkpoint = torch.load("DQN/policy_net.pt")
    ...
    #check if the last 3 images were perfectly approximated
    if all(difference == 0 for difference in final_differences[-5:]):
        #if so we consider the model trained
        Trained = True
    

#load checkpoint
checkpoint = torch.load("DQN/policy_net_trained.pt")
...
#set policy_net to eval mode for inference
policy_net.eval()

#create new environment using new image
env = Environment("TestImages/TestImage.jpg", NUM_SQUARES, device)
